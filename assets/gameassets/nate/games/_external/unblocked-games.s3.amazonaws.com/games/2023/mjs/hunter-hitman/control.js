...
var playbackControl = {
  ...
  resume: function () {
    ...
    var sfxIsOn = ig.game.load('sfx');
    var bgmIsOn = ig.game.load('bgm');
    if (sfxIsOn) {
      // unmute sfx
      if (typeof ig !== "undefined") {
...
function REWARDED_ADS_COMPLETED_COIN(isSuccess) {
  //call rewarded ads success function, true => success/completed, false => incompleted
  var btnReward = ig.game.getEntitiesByType(EntityButtonReward)[0];
  if (btnReward) btnReward.rewardedVideoResult(isSuccess);
}
function REWARDED_ADS_COMPLETED_LOOTBOX(isSuccess) {
  //call rewarded ads success function, true => success/completed, false => incompleted
  var lootboxClaim = ig.game.getEntitiesByType(ig.LootboxClaim)[0];
  if (lootboxClaim) {
    if (isSuccess) {
        // console.log("ad success : " + isSuccess);
        lootboxClaim.spawnPremiumCards();
    } else {
      lootboxClaim.fadeInAll()
    }
  }
}